...
# 문제 정의
AI 기반 시장성 분석 자동화 시스템을 설계하기 위해 먼저 시장 조사 과정의 핵심 문제를 분석했습니다. 기존 시장 조사는 비정형 데이터 수집과 정리에 많은 시간이 소요되며, 
실제로 2일에서 4~8주까지 걸려 프로젝트 진행을 지연시키는 일이 빈번했습니다. 또한 다양한 출처에서 얻은 데이터는 편향적이고 구조화되어 있지 않아 사람이 직접 해석해야 했습니다. 
LLM을 그대로 사용해 분석할 경우엔 요약 과정에서 허구 정보를 생성하는 Hallucination 문제가 발생해 결과의 신뢰성을 떨어뜨렸습니다. 
지인 인터뷰 및 질문 커뮤니티 사례에서도 시장 조사가 어렵고 시간이 오래 걸린다는 공통적인 어려움이 확인되었습니다. 
이런 문제들을 해결하기 위해 “AI 기반 자동 시장 분석 에이전트”가 필요했습니다.
...
## ⚠️ 프로젝트 구동 관련 안내 (Notice)
> **현재 클라우드 벡터 DB(Qdrant Cloud)의 무료 트라이얼 기간 만료로 인해 외부 API 실시간 연동 테스트는 제한됩니다.**
> 하지만 n8n 파이프라인, Upstage 임베딩 가이드라인, GPT-4o 기반의 Structured JSON 파서 등 **모든 핵심 백엔드 워크플로우 로직과 데이터 구조(Schema)는 리포지토리에 온전하게 보존**되어 있습니다. 
> 
> 로컬 환경(Docker)이나 대체 인프라(Chroma, Pinecone 등) 연결 시 즉시 재구동이 가능하도록 환경 변수(ENV) 및 파이프라인 모듈화가 완료된 상태입니다. 실구동 형태는 상단의 [6) 실행 영상]을 통해 확인하실 수 있습니다.
